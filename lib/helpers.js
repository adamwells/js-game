(function () {
	...
	var angleAtVertex = Helpers.angleAtVertex = function (vertex1, vertex2, vertex3) {
		...
		var angle = 1/Math.cos((Math.pow(side2, 2) + Math.pow(side3, 2) - Math.pow(side1, 2)) / 2 * side2 * side3);
		console.log(angle)
		return angle;
	}
	...
	var pointInsidePoly = Helpers.pointInsidePoly = function (point, polyVertices) {
		...
		sumAngles += angleAtVertex(point, polyVertices[polyVertices.length-1], polyVertices[0]);
		console.log('DONE')

		if (Math.abs(sumAngles) > Math.PI) {
			return true;
		}
